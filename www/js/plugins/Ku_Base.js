 ...
 * @param 最低ステータス
 * @desc ゲーム内でのアクターとエネミーのステータス最低値を変更します。初期値1※
 * @default 1
 ...
 * されるので少数までの設定を推奨。999.999と設定した時、エネミーHP999999
 * アクター側はHPとMPは9999、その他は999
...
Game_Action.prototype.makeDamageValue = function (target, critical) {
  ...
  if (target && target._states.contains($sid)) {
    ///ステート状態をチェック
...
Game_BattlerBase.prototype.attackTimesAdd = function () {
  return Math.max(
    this.traitsSumAll(Game_BattlerBase.TRAIT_ATTACK_TIMES) + this.agi / $agir,
    0
  );
};
