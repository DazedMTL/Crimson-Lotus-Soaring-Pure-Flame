...
(function () {
  ...
  function messageSpeedMax() {
    return toNumber(
      pMessageSpeedList[pMessageSpeedList.length - 1].MessageSpeed
    );
  }
  ...
  function messageSpeedStatusText(value) {
    const messageSpeed = pMessageSpeedList.find(
      (messageSpeed) => messageSpeed.MessageSpeed == value
    );
    if (messageSpeed) {
   ...
   * 【独自】メッセージ速度の値を変更
   */
  Window_Options.prototype.changeMessageSpeedValue = function (
    symbol,
    value,
    wrap
  ) {
  ...
  function nextMessageSpeed(value) {
    let messageSpeed = pMessageSpeedList.find(
      (messageSpeed) => messageSpeed.MessageSpeed == value
    );
