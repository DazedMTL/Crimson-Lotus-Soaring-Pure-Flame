...
Moghunter.b_motion_shake_effect_actor = String(
  Moghunter.parameters["Shake Effect Actor"] || "true"
);
Moghunter.b_motion_shake_effect_enemy = String(
  Moghunter.parameters["Shake Effect Enemy"] || "true"
);
Moghunter.b_motion_disable_blink_damage = String(
  Moghunter.parameters["Disable Blink Damage"] || "false"
);
...
Game_System.prototype.initialize = function () {
  ...
  if (String(Moghunter.b_motion_disable_blink_damage) === "true") {
...
Game_Action.prototype.prepare = function () {
  ...
  if (this.subject().isEnemy()) {
    this.subject().set_bmotion_action(this._item);
  }
};
...
// ** iniMembers
//==============================
var _alias_mog_bmotion_gbattler_initMembers =
...
Game_Battler.prototype.set_motion_data = function () {
  ...
  this._motion_idle_scale = [1.0, 1.0];
  ...
  this._motion_swing = [0, 0.003, 0.1, 60, 0.35, false];
};
...
Game_Battler.prototype.clear_action_data = function () {
  ...
  if (Imported.MOG_EnemyPoses && this._batPoses) {
    this._batPoses[2] = 1;
  }
};
...
Game_Battler.prototype.set_battler_motion_data = function () {
  for (var i = 0; i < this.notetags().length; i++) {
    if (this.notetags()[i] == "Breath Effect") {
      this._motion_breath[5] = true;
    }
    if (this.notetags()[i] == "Float Effect") {
      this._motion_fly[5] = true;
    }
    if (this.notetags()[i] == "Swing Effect") {
      this._motion_swing[5] = true;
    }
  }
};
...
// ** OnBattleStart
//==============================
var _alias_mog_bmotion_gbattler_onBattleStart =
...
Game_Battler.prototype.motion_Xaxis = function () {
  return (
    this._motion_idle_xy[0] +
    this._motion_action_xy[0] +
    this._motion_damage_xy[0]
  );
};
...
Game_Battler.prototype.motion_Yaxis = function () {
  return (
    this._motion_idle_xy[1] +
    this._motion_action_xy[1] +
    this._motion_damage_xy[1]
  );
};
...
Game_Battler.prototype.motion_ScaleX = function () {
  return (
    this._motion_idle_scale[0] +
    this._motion_action_scale[0] +
    this._motion_collapse_scale[0]
  );
};
...
Game_Battler.prototype.motion_ScaleY = function () {
  return (
    this._motion_idle_scale[1] +
    this._motion_action_scale[1] +
    this._motion_collapse_scale[1]
  );
};
...
Game_Battler.prototype.motion_rotation = function () {
  return (
    this._motion_idle_rotation +
    this._motion_action_rotation +
    this._motion_collapse_rotation
  );
};
...
Game_Battler.prototype.forceAction = function (skillId, targetIndex) {
  ...
  if (this._actions[0]) {
    this.set_bmotion_action(this._actions[0]._item);
  }
};
...
Game_Action.prototype.apply = function (target) {
  ...
  if (this.item() && this.item().damage.type != 1) {
    target._motion_damage_duration = 0;
  }
};
...
// * create Enemies
//==============================
var _alias_mog_bmotion_spriteseBattle_createEnemies =
...
// * iniVisibility
//==============================
var _alias_mog_bmotion_spenemy_initVisibility =
...
Sprite_Enemy.prototype.updateBitmap = function () {
  ...
  if (this._check_duration < 10) {
    this._check_duration += 1;
  }
  if (this._check_duration > 1) {
    if (this._check_initsetup && this.bitmap.isReady()) {
      this.set_bmotion_setup();
    }
  }
};
...
Sprite_Battler.prototype.set_bmotion_setup = function () {
  ...
  if (this._battler.is_breath_mode()) {
    this.set_breath_mode();
  }
  if (this._battler.is_fly_mode()) {
    this.set_fly_mode();
  }
  if (this._battler.is_swing_mode()) {
    this.set_swing_mode();
  }
};
...
Sprite_Battler.prototype.set_breath_mode = function () {
  ...
  if (this.bitmap.height <= 100) {
    this._battler._motion_breath[4] = 1;
  }
  if (this.bitmap.height >= 300) {
    this._battler._motion_breath[4] = 3;
  }
...
Sprite_Battler.prototype.set_fly_mode = function () {
  ...
  if (!Imported.MOG_EmergeMotion) {
    this._battler._motion_idle_xy[1] = -Math.floor(
      Math.random() * this._battler._motion_fly[3]
    );
  }
};
...
Sprite_Battler.prototype.set_swing_mode = function () {
  this._battler._motion_swing = [0, 0.003, 0.1, 0, 0.35, true];
  var rz = (Math.random() * 0.1).toFixed(2);
...
// * Update Position
//==============================
var _alias_mog_battlerMotion_sprbtr_updatePosition =
...
Sprite_Battler.prototype.update_bmotion = function () {
  if (this._spriteShadow != null) {
    this._spriteShadow.update_shadow(this);
  }
...
Sprite_Battler.prototype.update_motion_idle = function () {
  ...
  if (this._battler.is_breath_mode()) {
    this.update_idle_breath_mode();
  }
  if (this._battler.is_fly_mode()) {
    this.update_idle_fly_mode();
  }
  if (this._battler.is_swing_mode()) {
    this.update_idle_swing_mode();
  }
};
...
Sprite_Battler.prototype.update_idle_breath_mode = function () {
  ...
  if (this._battler._motion_breath[2] < this._battler._motion_breath[4]) {
    return;
  }
  ...
  if (this._battler._motion_breath[0] == 0) {
    ...
    if (this._battler._motion_idle_scale[1] > this._battler._motion_breath[3]) {
      this._battler._motion_idle_scale[1] = this._battler._motion_breath[3];
      this._battler._motion_breath[0] = 1;
    }
  } else {
    this._battler._motion_idle_scale[1] -= this._battler._motion_breath[1];
    if (this._battler._motion_idle_scale[1] < 1.0) {
      this._battler._motion_idle_scale[1] = 1.0;
      this._battler._motion_breath[0] = 0;
    }
  }
};
...
Sprite_Battler.prototype.update_idle_fly_mode = function () {
  ...
  if (this._battler._motion_fly[4] < 3) {
    return;
  }
  this._battler._motion_fly[4] = 0;
...
Sprite_Battler.prototype.update_idle_swing_mode = function () {
  if (this._battler._motion_swing[0] === 0) {
    ...
    if (this._battler._motion_idle_rotation >= this._battler._motion_swing[2]) {
      this._battler._motion_idle_rotation = this._battler._motion_swing[2];
      this._battler._motion_swing[0] = 1;
    }
  } else {
    this._battler._motion_idle_rotation -= this._battler._motion_swing[1];
    if (
      ...
      this._battler._motion_idle_rotation = -this._battler._motion_swing[2];
      this._battler._motion_swing[0] = 0;
    }
  }
};
...
Sprite_Battler.prototype.update_action_swing_right = function () {
  ...
  if (this._battler._motion_action_data[1] < 10) {
    ...
    this._battler._motion_action_scale[0] = -2.0;
    this._battler._motion_action_xy[0] -= 6;
  } else if (this._battler._motion_action_data[1] < 40) {
    this._battler._motion_action_scale[0] = 0;
    this._battler._motion_action_xy[0] += 6;
...
SpriteBattlerShadow.prototype.loadFiles = function (sprite) {
  if (!sprite._battler.is_fly_mode()) {
    return;
  }
...
SpriteBattlerShadow.prototype.update_shadow = function (sprite) {
  ...
  this.x = sprite.x;
  ny = 0;
  if (this.need_set_ny_action(sprite._battler)) {
    ny = sprite._battler._motion_action_xy[1];
  }
...
SpriteBattlerShadow.prototype.need_set_ny_action = function (battler) {
  if (battler._motion_action_data[0] == 4) {
    return true;
  }
  return false;
};
