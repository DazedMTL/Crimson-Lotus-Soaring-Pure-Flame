...
/*:
 * @plugindesc v1.3.6 バトルイベントを拡張するプラグイン　YEP_VictoryAftermathより下である必要がある（PL追記）
 * @author フトコロ
 *
 * @param Battle Event
...
// プラグイン パラメータ
//=============================================================================
var parameters = PluginManager.parameters("FTKR_ExBattleEvent_custom");
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
		console.log('FTKR_ExBattleEvent_custom.js:pluginCommand');
...
BattleManager.checkBattleEnd = function () {
  //console.log('FTKR_ExBattleEvent_custom.js:checkBattleEnd');
  if (this.isBattleEndEvent()) {
    this._checkEbeBattleEvent = false;
    switch (this._battleEndPattern) {
      case 0: //勝利
        if (FTKR_ExBattleEvent_btlEnd_ev_enable) {
          if (FTKR.EBE.battleEnd.customV) break;
          FTKR_ExBattleEvent_btlEnd_ev_enable = false;
...
BattleManager.processVictory = function () {
  //console.log('FTKR_ExBattleEvent_custom.js:processVictory');
  if (FTKR.EBE.battleEnd.victory && !this.isBattleEndEvent()) {
    //console.log('FTKR_ExBattleEvent_custom.js:processVictory:isBattleEndEvent');
...
BattleManager.processDefeat = function () {
  //console.log('FTKR_ExBattleEvent_custom.js:processDefeat');
...
BattleManager.checkAbort = function () {
  //console.log('FTKR_ExBattleEvent_custom.js:checkAbort');
...
Game_Troop.prototype.setupEbeBattleEvent = function (condition, metacodes) {
  //console.log('FTKR_ExBattleEvent_custom.js:setupEbeBattleEvent');
  if (!this._interpreter.isRunning()) {
    if (this._interpreter.setupReservedCommonEvent()) {
      return false;
    }
    ...
    if (FTKR_ExBattleEvent_btlEnd_commonEV_enable) {
      FTKR_ExBattleEvent_btlEnd_commonEV_enable = false;
...
var FTKR_ExBattleEvent_btlEnd_commonEV_enable = false; // 他のプラグインとの兼ね合いで戦闘終了処理のコモンイベントが連続して行われるのを防ぐ
var FTKR_ExBattleEvent_btlEnd_ev_enable = false; // YEP_VictoryAftermath.jsとの競合で1度だけ処理したい場合に使用
(function () {
  var parameters = PluginManager.parameters("FTKR_ExBattleEvent_custom");
  var C_EV_BTL_START = parameters["Battle Start Event"];

  // 戦闘開始時
  var _BattleManager_startBattle = BattleManager.startBattle;
  BattleManager.startBattle = function () {
    //console.log("startBattle:"+C_EV_BTL_START);
    FTKR_ExBattleEvent_btlEnd_commonEV_enable = true;
    FTKR_ExBattleEvent_btlEnd_ev_enable = true;
    $gameTemp.reserveCommonEvent(C_EV_BTL_START);
